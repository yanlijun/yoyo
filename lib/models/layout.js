...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      //todo: 当前系统的权限 and 初始化报错
      dispatch({
        type: 'queryUser',
      })
    },
    matchPath({ dispatch, history }) {
      return history.listen(({ pathname }) => {
        _.each(window._matchPathMap, (val, key) => {
          const re = matchPath(location.pathname, {
            path: key,
          })
          if (re) {
            val.forEach((action) => {
  ...
  reducers: {
    ...
    routeChange(state, { payload }) {
      ...
      let currentMenu = _.find(state.menuMap, (val, key) => {
        return matchPath(payload.pathname, {
          path: val.url,
          exact: true,
        })
      })
